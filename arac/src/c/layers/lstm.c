...
void layer_forward(Layer* layer_p, LstmLayer* ll_p)
{
    ...
    if ((*layer_p->timestep_p) > 0)
    {
        ...
        memcpy((void*) (mdlstm_p->inputs.contents_p + size),
    ...
    memcpy((void*) (ll_p->states.contents_p + offset), 
           (void*) (mdlstm_p->outputs.contents_p + layer_p->inputs.size),
           sizeof(double) * size);
}
...
void layer_backward(Layer* layer_p, LstmLayer* ll_p)
{
    ...
    offset = ((*layer_p->timestep_p) - 1) * layer_p->outputs.size;
    memcpy((void*) (mdlstm_p->outputs.error_p),
           (void*) (layer_p->outputs.error_p + offset),
           sizeof(double) * layer_p->outputs.size);

    offset = ((*layer_p->timestep_p)) * layer_p->outputs.size;
    ...
    memcpy((void*) (ll_p->states.error_p  + offset),
           (void*) (mdlstm_p->inputs.error_p + layer_p->inputs.size),
           sizeof(double) * size);
}
