...
class AbstractMCSVM(Module,Dumpable):
    ...
    def __init__( self, indim, outdim, kernel=KT.RBF, **kwargs ):
        ...
        The passed kernel will be used for all submodules. See the 
        SVM.__init__() documentation for more information."""
    ...
    def classifyDataSet(self, dataset):
        """Return the predicted classes for an entire data set."""
        dataset.reset()
        self.reset()
    ...
    def rawOutputs(self,x):
        """Returns a vector of raw outputs regarding input vector x of all sub
...
class MCSVMOneAgainstOne(AbstractMCSVM):
    ...
    def _setData(self, dataset):
        ...
        The samples will be separated into subsets of same class. Then each SVM
        sub module will be fed with data containing samples of just the two
        classes that were assigned to it."""
        ...
        for i in range(classnum):
            for j in range( i+1, classnum ):
                sub_module = SVM(indim, outdim, 
                                 self._kernel, 
                                **self._sub_kernel_kwargs)
