...
class Dumpable(object):
    """ Base Class that provides saving and loading of objects of derived 
    Classes to/from a file. """
    @classmethod
...
def vectorizeX(func):
    """ Decorator for functions that expect one input vector x, but should also 
    be able to expect an array of vectors. If x is a single vector, the regular
    function will be called and its return value be forwarded.
...
class SVM(Module,Dumpable):
    ...
    to separate the input vectors of TWO classes. For data with more than two
    classes use the MCSVM module.
    
    Train the SVM module by associating it to an instance of SVMTrainer.

    You can save an instance of SVM to a file by calling the inhertited method
    saveToFile(). For loading use loadFromFile()."""

    def __init__( self, indim, outdim, kernel=KT.RBF, **kwargs ):
        """ Initialize the SVM module with input dimensionality indim and output
        dimensionality outdim, which should always be 1. 
        
        For a specific kernel function, pass the appropriate constant as kernel.
        Any additional arguments will be directed to the kernel class. For more
        information, see the corresponding class."""
    ...
    def setParams(self,**kwargs):
        """ Set the module's parameters. These will mostly be redirected to the
        kernel. See Kernel for more information about kwargs."""
    ...
    def _forwardImplementation(self, inbuf, outbuf):
        """Forward pass method of a trained support vector machine. It 
        classifies the input vector stored in inbuf, and writes the class label
        to outbuf. Note that the length of the outbuf vector should always be 1.
        """
    ...
    def _setDataXY(self, X, Y):
        """Same as _setData except you have to supply an array X with input data
        and an array Y with target data instead of a SupervisedDataSet."""
        classes = list(set(Y))
        self._classes = classes
        if len(classes) > 2: 
            raise Exception("Too many classes detected. Use MCSVM instead.")
    ...
    def rawOutput( self, x ):
        """ Return the raw output of the trained SVM model, without classifying 
        it.
        
        The output is the scaled distance of the input vector x to the 
    ...
    @vectorizeX
    def classify( self, x ):
        """Return the classified output of the trained SVM model regarding the
        input vector x. This is done by calculating the raw output of the svm 
        for input x, and then mapping the sign of this value to the respective 
        class label.
...
class AbstractMCSVM(Module,Dumpable):
    ...
    def __init__( self, indim, outdim, kernel = KT.RBF, **kwargs ):
        ...
        kernel will be used for all sub modules.
        """
        Module.__init__( self, indim, outdim )
    ...
    def setParams(self, **kwargs):
        """ Set the module's parameters. See SVM.setParams() for more 
        information."""
        for key, value in kwargs.items():
            if key in ("verbose", "ver", "v"):
                self._verbosity = value
...
#            self.getSubModules = self._getSubModulesOneAgainstAll
    ...
    def rawOutputs(self,x):
        """Return a vector of raw outputs regarding input vector x of all sub 
        modules, in the order [0-1, 0-2, ..., 0-n, 1-2, 1-3, ..., (n-1)-n].
        
        The length of this vector can be retrieved by the nSubModules() method.
        """
    ...
    def _forwardImplementation( self, inbuf, outbuf ):
        """ Forward pass method, that classifies the input vector inbuf and 
        writes the label to outbuf. See SVM._forwardImplementation()
        documentation."""
        outbuf[0] = self.classify(inbuf)
    ...
    def _setData( self, dataset ):
        ...
        for i in range(classnum):
            for j in range( i+1, classnum ):
                sub_module = \
                    SVM(indim, outdim, self._kernel, **self._sub_kernel_kwargs)
...
class MCSVMrawPair(MCSVM):
    """Multi-class SVM that returns raw outputs for all pairwise
    sub-classifiers."""
...
class Kernel:
    ...
    def __init__(self,**kwargs):
        ...
        See .setParams() for a description of kwargs."""
        self._X = []
        self._Y = []
        self._degree = 3.
        self._gamma  = 1.
    ...
    def setParams(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key in ("degree"):         
                self._degree = float(value)
            elif key in ("gamma"):        
                self._gamma  = float(value)
            elif key in ("coef0"):        
                self._coef0  = float(value)

    def k(self, x1, x2):
        """Abstract kernel method that has to be implemented.

        Calculates the inner product of the input vectors x1 and x2, after
        transforming them into feature space:

            <phi(x1), phi(x2)>
        
        x1 is a single sample or an array of samples, while x2 is a single 
        sample."""
        raise NotImplementedError()
    ...
    def setData(self,X,Y):
        ...
        X is a 2D numpy array with inputvectors of same length while Y is a 1D 
        numpy array with corresponding target values of the inputvectors.
        """
        self._X = X
        self._Y = Y
        self.l = len(Y)
    ...
    def isExplicit(self):
        """ Returns wether this kernel object has an explicit phi method for
        transforming an input vector into feature space.
...
class RBFKernel(Kernel):
    ...
    def k( self, x1, x2 ):
        ...
        xd = x1-x2
        if xd.ndim <= 1:
            return exp(- self._gamma * dot(xd,xd))
        else:
            return exp(- self._gamma * numpy.sum(xd**2, axis=1))
...
class ExplicitKernel(Kernel):
    ...
    def getFeatureSpaceDim(self):
        """Return dimensionality of feature space defined by the phi-function.
        """
